...
function AtomComponent({atom_idx= 1, atom_weight=false}) {
    ...
    useEffect(() => {
        ...
        fetch(apiURL)
            ...
            .then(data => {
                ...
                setRenderSelf(true)
                console.log('Done fetching from: ', apiURL)
            })
            .catch(error => {
...
// function CodeComponent({code_idx=100}) {
//     const [codeData, setCodeData] = useState([{atom: 'Loading...', weight: 255.}])
//     const [maxWeight, setMaxWeight] = useState(255)

//     useEffect(() => {
//         const apiURL = "https://tok-embed-nnmf-api-489e56c40ef7.herokuapp.com/code/"+code_idx
//         fetch(apiURL)
//             .then(response => {
//                 if (!response.ok) {
//                     throw new Error('Network response was not ok')
...
//     return (
//         <>
//             <h2 style={{'textAlign': 'left'}}>Code {code_idx}</h2>
//             {codeData.map((dict, idx) => {
//                 return (
//                     // <span key={idx}>{dict['atom']}</span>
//                     <WrappedAtomComponent key={idx} atom_idx={dict['atom']} atom_weight={dict['weight']} />
//                 )
//             })}
//         </>
//     )
...
function CodeComponent({codeStringCandidate=' hello'}) {
    ...
    return (
        <div style={{width: '100%', textAlign: 'left'}}>
            <h2 style={{'textAlign': 'left'}}>Code {codeIdx}: "{codeStr}"</h2>
